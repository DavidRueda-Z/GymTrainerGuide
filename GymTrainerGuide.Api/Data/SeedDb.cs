...
namespace GymTrainerGuide.Api.Data
{
    public class SeedDb
    {
        ...
        private async Task<Usuario> CheckUserAsync(string nombre, int edad, Genero genero, string correo, TipoUsuario tipoUsuario, int nivelExperienciaId, int objetivoId, string password)
        {
        ...
        private async Task CheckRutinaAsync()
        {
            if (!_context.Rutinas.Any())
            {
                ...
                var usuarioAna = await _userHelper.GetUserAsync("ana.garcia@gym.com");
                if (usuarioAna != null)
                {
